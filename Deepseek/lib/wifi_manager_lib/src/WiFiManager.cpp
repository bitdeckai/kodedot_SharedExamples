...
String DEEPSEEK_API_KEY_STR;
String API_PREFER_STR;   // "openai" or "deepseek", lowercase
String BAIDU_ASR_AUTH_KEY_STR;
String BAIDU_ASR_API_KEY_STR;
String BAIDU_ASR_SECRET_KEY_STR;
...
bool WiFiManager::loadApiKeysFromSD() {
    ...
    while (apis.available()) {
        ...
        } else if (key.equalsIgnoreCase("DEEPSEEK")) {
            DEEPSEEK_API_KEY_STR = val;
            Serial.printf("[WiFiManager] DEEPSEEK key loaded (%u chars)\n", (unsigned)DEEPSEEK_API_KEY_STR.length());
        } else if (key.equalsIgnoreCase("BAIDU_ASR_AUTH_KEY") || key.equalsIgnoreCase("BAIDU_AUTH_KEY")) {
            BAIDU_ASR_AUTH_KEY_STR = val;
            Serial.printf("[WiFiManager] BAIDU ASR auth key loaded (%u chars)\n", (unsigned)BAIDU_ASR_AUTH_KEY_STR.length());
        } else if (key.equalsIgnoreCase("BAIDU_ASR_API_KEY") || key.equalsIgnoreCase("BAIDU_API_KEY")) {
            BAIDU_ASR_API_KEY_STR = val;
            Serial.printf("[WiFiManager] BAIDU ASR API key loaded (%u chars)\n", (unsigned)BAIDU_ASR_API_KEY_STR.length());
        } else if (key.equalsIgnoreCase("BAIDU_ASR_SECRET_KEY") || key.equalsIgnoreCase("BAIDU_SECRET_KEY")) {
            BAIDU_ASR_SECRET_KEY_STR = val;
            Serial.printf("[WiFiManager] BAIDU ASR Secret key loaded (%u chars)\n", (unsigned)BAIDU_ASR_SECRET_KEY_STR.length());
        } else if (key.equalsIgnoreCase("PREFER")) {
            API_PREFER_STR = val;
            Serial.printf("[WiFiManager] Preference set to '%s'\n", API_PREFER_STR.c_str());
        }
    }
    apis.close();

    const bool hasBaiduAsr = BAIDU_ASR_AUTH_KEY_STR.length() > 0 ||
                             (BAIDU_ASR_API_KEY_STR.length() > 0 && BAIDU_ASR_SECRET_KEY_STR.length() > 0);
    if (OPENAI_API_KEY_STR.length() == 0 && DEEPSEEK_API_KEY_STR.length() == 0 && !hasBaiduAsr) {
        Serial.println("[WiFiManager] WARNING: No API key found in /apis.txt");
        return false;
    }
    return true;
}
