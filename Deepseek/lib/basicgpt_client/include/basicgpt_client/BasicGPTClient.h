...
public:
    struct Config {
        ...
        // HTTP endpoint path (including leading slash). OpenAI defaults to
        // "/v1/chat/completions"; Deepseek or other backends may require
        // a different path, so this is configurable.
        const char* endpointPath = "/v1/chat/completions";
        bool        stream = false;            // request streaming output (compatible with Deepseek example)
    ...
    // Sends a plain text chat request and fills outText with assistant reply.
    bool askText(const String& userText, String& outText);

    // Same as askText but with conversation history for context.
    bool askTextWithHistory(const String& userText,
...
private:
    ...
    static String readResponseBodySafe(Client& client, uint32_t timeoutMs, size_t maxBytes);
