...
String BasicGPTClient::extractAssistantText(const String& body) {
    ...
    if (choicesPos >= 0) {
        int messagePos = body.indexOf("\"message\"", choicesPos);
        if (messagePos >= 0) {
            int contentPos = body.indexOf("\"content\"", messagePos);
            if (contentPos >= 0) {
                int q1 = body.indexOf('"', contentPos + 9);
                int q2 = -1;
                if (q1 >= 0) {
...
String BasicGPTClient::readResponseBodySafe(Client& client, uint32_t timeoutMs, size_t maxBytes) {
    String body;
    body.reserve(maxBytes > 8192 ? 8192 : maxBytes);

    uint32_t idleStart = millis();
    while (true) {
        int byteValue = client.read();
        if (byteValue >= 0) {
            body += static_cast<char>(byteValue);
            idleStart = millis();
        ...
        if (millis() - idleStart > timeoutMs) {
            Serial.println("[BasicGPTClient] WARNING: timed out waiting for response body");
            break;
        }
...
bool BasicGPTClient::askText(const String& userText, String& outText) {
    std::vector<ChatMessage> emptyHistory;
    return askTextWithHistory(userText, emptyHistory, outText);
}

bool BasicGPTClient::askTextWithHistory(const String& userText,
    ...
    String body = String("{\"model\":\"") + cfg_.model + "\",";
    body += "\"messages\":[";

    bool needComma = false;
    if (cfg_.systemPrompt && cfg_.systemPrompt[0] != '\0') {
        body += "{\"role\":\"system\",\"content\":\"" + jsonEscape(String(cfg_.systemPrompt)) + "\"}";
        needComma = true;
    }

    for (size_t i = 0; i < conversationHistory.size(); ++i) {
        if (needComma) body += ",";
        body += "{\"role\":\"" + conversationHistory[i].role + "\",\"content\":\"" + jsonEscape(conversationHistory[i].content) + "\"}";
        needComma = true;
    }
    ...
    Serial.printf("[BasicGPTClient] connecting to %s:%d\n", cfg_.host, cfg_.port);
    if (!client.connect(cfg_.host, cfg_.port)) {
        Serial.println("[BasicGPTClient] ERROR: TLS connect failed");
        return false;
    }
    Serial.println("[BasicGPTClient] TLS connection established");

    client.printf("POST %s HTTP/1.1\r\n", cfg_.endpointPath ? cfg_.endpointPath : "/");
    ...
    Serial.printf("[BasicGPTClient] text body length=%u\n", (unsigned)body.length());
    client.print(body);
    ...
    if (!httpOk) {
        Serial.print("[BasicGPTClient] HTTP status: ");
    ...
    String responseBody = readResponseBodySafe(client, cfg_.httpTimeoutMs, 65536);
    client.stop();

    outText = extractAssistantText(responseBody);
    if (outText.length() == 0) {
...
bool BasicGPTClient::askAudioFromPCM(const uint8_t* pcmData,
    ...
    // Build suffix; include stream flag if requested (must come after messages)
    String suf;
    if (cfg_.stream) {
        suf = String(",\"format\":\"wav\"}}]},\"stream\":true}");
    } else {
        suf = String(",\"format\":\"wav\"}}]},\"stream\":false}");
    }
    ...
    // HTTP headers (path configurable via cfg_.endpointPath)
    client.printf("POST %s HTTP/1.1\r\n", cfg_.endpointPath ? cfg_.endpointPath : "/");
    client.printf("Host: %s\r\n", cfg_.host);
    client.print ("Content-Type: application/json\r\n");
    client.print ("Accept: application/json\r\n");
    ...
    client.print(suf);
    Serial.printf("[BasicGPTClient] finished sending body (length=%u)\n", (unsigned)contentLen);
    Serial.printf("[BasicGPTClient] client.connected=%d available=%d\n", client.connected(), client.available());
    free(inBuf);
    free(outBuf);
    ...
    if (status.length() == 0) {
        Serial.println("[BasicGPTClient] ERROR: Empty HTTP status line");
        Serial.printf("[BasicGPTClient] connected=%d available=%d\n", client.connected(), client.available());
        int bytes = client.available();
        if (bytes > 0) {
            String raw = client.readString();
            Serial.printf("[BasicGPTClient] raw data after empty status: %s\n", raw.c_str());
        }
    }
    bool httpOk = (status.indexOf(" 200 ") >= 0 || status.startsWith("HTTP/1.1 200") || status.startsWith("HTTP/1.0 200"));
    if (!httpOk) {
    ...
    String body = readResponseBodySafe(client, cfg_.httpTimeoutMs, 65536);
    client.stop();
...
bool BasicGPTClient::askAudioFromWav(const char* wavPath, String& outText) {
    ...
    while (client.connected()) {
        ...
        } else if (millis() - idleStart2 > cfg_.httpTimeoutMs) {
            Serial.println("[BasicGPTClient] WARNING: timed out waiting for response body");
            break;
        } else {
            delay(10);
