...
size_t utf8NextBoundary(const String& text, size_t offset) {
    if (offset >= text.length()) {
        return text.length();
    }
    const size_t next = offset + utf8CharLength(static_cast<uint8_t>(text[offset]));
    return next > text.length() ? text.length() : next;
}

uint32_t utf8DecodeCodePoint(const String& text, size_t offset) {
    if (offset >= text.length()) {
        return 0;
    }

    const uint8_t lead = static_cast<uint8_t>(text[offset]);
    const size_t len = utf8CharLength(lead);
    if (len == 1 || offset + len > text.length()) {
        return lead;
    }
    ...
    if (len == 3) {
        return ((lead & 0x0FU) << 12) |
               ((static_cast<uint8_t>(text[offset + 1]) & 0x3FU) << 6) |
               (static_cast<uint8_t>(text[offset + 2]) & 0x3FU);
    }
    return ((lead & 0x07U) << 18) |
           ((static_cast<uint8_t>(text[offset + 1]) & 0x3FU) << 12) |
           ((static_cast<uint8_t>(text[offset + 2]) & 0x3FU) << 6) |
           (static_cast<uint8_t>(text[offset + 3]) & 0x3FU);
}

bool isLongPauseCodePoint(uint32_t codePoint) {
    return codePoint == '.' || codePoint == '!' || codePoint == '?' ||
           codePoint == 0x3002 || codePoint == 0xFF01 || codePoint == 0xFF1F;
}

bool isMediumPauseCodePoint(uint32_t codePoint) {
    return codePoint == ',' || codePoint == ';' || codePoint == ':' ||
           codePoint == 0xFF0C || codePoint == 0x3001 ||
           codePoint == 0xFF1B || codePoint == 0xFF1A;
}

bool isSpaceCodePoint(uint32_t codePoint) {
    return codePoint == ' ' || codePoint == 0x3000;
}

void copyUtf8Safely(char* dest, size_t capacity, const char* src) {
    if (!dest || capacity == 0) {
        return;
    }

    size_t written = 0;
    while (src && *src && written < capacity - 1) {
        const size_t charLen = utf8CharLength(static_cast<uint8_t>(*src));
        if (written + charLen > capacity - 1) {
            break;
        }

        size_t copied = 0;
        while (copied < charLen && src[copied] != '\0') {
            dest[written++] = src[copied++];
        }
        if (copied < charLen) {
            break;
        }
...
void UIManager::postStatus(const char* text) {
    ...
    copyUtf8Safely(msg.text, sizeof(msg.text), text);
...
void UIManager::setState(UIState state) {
    ...
    // Update eye color based on state (kept for compatibility)
    updateEyeColor(state);

    // Handle different states
    if (state == UIState::ShowingResponse) {
        // Hide state image when showing text response
        if (stateImage_) {
            lv_obj_add_flag(stateImage_, LV_OBJ_FLAG_HIDDEN);
        }
        if (thinkingTimer_) {
            lv_timer_del(thinkingTimer_);
            thinkingTimer_ = nullptr;
        ...
        // Show Start image when ready
        if (stateImage_) {
            lv_image_set_src(stateImage_, &Start);
            lv_obj_clear_flag(stateImage_, LV_OBJ_FLAG_HIDDEN);
        }
        ...
        // Show Start image while processing
        if (stateImage_) {
            lv_image_set_src(stateImage_, &Start);
            lv_obj_clear_flag(stateImage_, LV_OBJ_FLAG_HIDDEN);
        }
        ...
        // Logo: animate while thinking
        setGPTLogoVisible(true);
        startGPTLogoAnimation(250); // slower thinking animation
    } else if (state == UIState::Recording) {
        // Show Listen image when recording
        if (stateImage_) {
            lv_image_set_src(stateImage_, &Listen);
            lv_obj_clear_flag(stateImage_, LV_OBJ_FLAG_HIDDEN);
        }
        if (thinkingTimer_) {
            lv_timer_del(thinkingTimer_);
        ...
        // Other states (Connecting, Error) - show Start image
        if (stateImage_) {
            lv_image_set_src(stateImage_, &Start);
            lv_obj_clear_flag(stateImage_, LV_OBJ_FLAG_HIDDEN);
        }
...
void UIManager::createUI() {
    ...
    // Create state image (Start/Listen) in center
    stateImage_ = lv_image_create(scr);
    lv_image_set_src(stateImage_, &Start);  // Default to Start image
    lv_obj_center(stateImage_);
    
    // Create eyes (kept for compatibility but hidden)
    createEyes();
    showEyes(false);  // Hide eyes by default
...
void UIManager::updateTypewriterText() {
    ...
    const size_t previousIndex = currentCharIndex_;
    currentCharIndex_ = utf8NextBoundary(pendingText_, currentCharIndex_);
    String currentText = pendingText_.substring(0, currentCharIndex_);
    ...
    const uint32_t currentCodePoint = utf8DecodeCodePoint(pendingText_, previousIndex);
